var github_user = function(username, callback) {
    fetch('https://api.github.com/users/' + username + '?callback=?').then(r => r.json()).then(callback);
}
...
var github_user_repos = function(username, callback, page_number, prev_data) {
    ...
    fetch(url).then(r => r.json()).then(function(repos) {
...
var github_user_orgs = function(username, callback) {
    fetch('https://api.github.com/users/' + username + '/orgs?callback=?').then(r=> r.json()).then(callback);
}
...
fetch(url).then((r) => {
    ...
    //     if (repo.full_name == "NeonGamerBot-QK/resume-action") {
    //         // star = true;
...
module.exports = (username, outputfile, error) => {
 var itemCount = 0,
maxItems = 5,
maxLanguages = 9,
...
const fileData= fs.readFileSync(resume).toString()
const html = ejs.render(fileData, view)
// return html;
// convert to pdf
// etc
// $.ajax({
...
$.ajax({
    ...
    success: function(response) {
        ...
        if (languages && languages.length > 0) {
            ...
            languages.forEach(function(i, lang) {
                x = i + 1;
